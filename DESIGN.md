...
**The Touch Floor Rule.** Under a coarse pointer every standalone control is at least 44×44: chips, facet rows, text actions, pagination, the chart toolbar and zoom buttons, the map's controls, the data-table disclosure, a four-letter `Cite`. The step is keyed to the input method (`--touch`), never to viewport width, and each control takes it in its own sheet, placed after any rule of equal specificity that would undo it. Two things are exempt by decision, not oversight. A link set inside a line of text (the DOI or `[Link]` closing a CV citation, a prose cross-reference) keeps its line height: WCAG 2.5.8 exempts inline targets, and a 44px box would break the line it sits in. An SVG data mark (a matrix cell, a graph node) keeps the size its datum gives it, and its data table is the equivalent. A control that merely sits beside text is not inline and takes the floor. `responsive.spec.ts` measures the index chips and both visualisation pages under touch.
