...
def extract_bigrams(text: str, nlp, top_n: int = 50) -> list:
    """Extract frequent bigrams (two-word phrases)."""
...
def generate_index_file(publication_ids: list[str]) -> Path:
    """Generate the index.ts that exports all publication analyses."""
    output_dir = get_output_dir()

    # Read existing files to include them too
    existing_files = list(output_dir.glob('*.ts'))
    existing_ids = {f.stem for f in existing_files if f.stem != 'index'}

    all_ids = sorted(existing_ids | set(publication_ids))
    ...
    for pub_id in all_ids:
        ...
        var_name = ''.join(
            ...
            for i, word in enumerate(pub_id.replace('-', ' ').split())
        ).replace(' ', '')
        imports.append(f"import {{ analysis as {var_name} }} from './{pub_id}';")
...
def process_single_file(
    ...
    bigrams = extract_bigrams(text, nlp)
...
def main():
    ...
    if processed_ids:
        print("\n" + "=" * 60)
        index_path = generate_index_file(processed_ids)
        print(f"Generated index: {index_path}")
        print(f"\nDone! Processed {len(processed_ids)} publication(s).")
