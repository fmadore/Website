import type { Communication } from '$lib/types/communication';

export const zmoKitchenTalksAiIslamicAfricanStudies: Communication = {
	id: 'zmo-kitchen-talks-ai-islamic-african-studies',
	title: 'Artificial Intelligence (AI) in African and Islamic Studies',
	authors: ['Elisa Nobel-Dilaty', 'Frédérick Madore'],
	date: '11 December 2025',
	dateISO: '2025-12-11',
	year: 2025,
	conference: 'ZMO Kitchen Talks',
	episode: 14,
	location: 'Leibniz-Zentrum Moderner Orient',
	country: 'Germany',
	type: 'podcast',
	language: 'English',
	abstract:
		'Dr. Frédérick Madore, postdoctoral research fellow at ZMO Berlin, works at the intersection of African Studies, Islamic Studies, and the digital humanities. Building on fieldwork and archival research conducted since 2013, he is developing the Islam West Africa Collection—an open-access digital archive of more than 14,500 documents on Islamic print culture and Muslim life in West Africa. In this episode, we explore how artificial intelligence is transforming research practices: How can AI support historical and anthropological research—and where are its limits? And how might AI change the future of the humanities—opening up new possibilities while also raising pressing questions about language equity, digitisation gaps, and academic responsibility?',
	tags: ['Podcast', 'AI', 'African Studies', 'Islamic Studies', 'Digital Humanities', 'IWAC', 'West Africa', 'Islam', 'ZMO'],
	url: 'https://www.zmo.de/en/kitchen-talks',
	additionalUrls: [
		{ label: 'Spotify', url: 'https://open.spotify.com/episode/3zScd0lM8ecPYnlV8wInDu?si=POACrwM7SlCB7HvRU1zEDg' },
		{ label: 'Podigee', url: 'https://zmo-kitchen-talks.podigee.io/14-neue-episode' },
		{ label: 'Apple Podcasts', url: 'https://podcasts.apple.com/us/podcast/artificial-intelligence-ai-in-african-and-islamic-studies/id1854423734?i=1000740827248' },
		{ label: 'Pocket Casts', url: 'https://pca.st/episode/dd8e2cc4-a1e4-4121-8b0b-2bdbf7e714d7' },
		{ label: 'Deezer', url: 'https://link.deezer.com/s/31RQUHFM3mhYI88kbzEW9' }
	],
	image: 'images/communications/zmo-kitchen-talks-logo.webp',
	heroImage: {
		src: 'images/communications/zmo-kitchen-talks-logo.webp',
		alt: 'ZMO Kitchen Talks podcast logo.'
	},
	coordinates: {
		latitude: 52.42814222811816,
		longitude: 13.202338657893593
	},
	project: 'Digital Humanities and AI in African Studies'
};
