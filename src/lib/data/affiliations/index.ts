...
type AffiliationModuleType = Record<
	string,
	{ default: ProfessionalAffiliation } | ProfessionalAffiliation
>;
...
const affiliationModules = import.meta.glob<AffiliationModuleType>(['./!(template|index).ts'], {
	eager: true
});
...
export const allAffiliations: ProfessionalAffiliation[] = Object.entries(affiliationModules)
	.map(([path, moduleContent]) => {
		...
		if (
			...
			typeof moduleContent.default === 'object' &&
			moduleContent.default !== null &&
			'id' in moduleContent.default
		) {
		...
		if (!affiliationData || !affiliationData.id) {
			console.warn(
				`Skipping module ${path} as it does not seem to export a valid affiliation or has no id.`
			);
			return null;
		}
