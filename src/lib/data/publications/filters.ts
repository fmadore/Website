...
export const filteredPublications = derived([activeFilters], ([$activeFilters]) => {
...
export const languageCounts = createDerivedCountStore(filteredPublications, (pub) =>
	pub.language?.split(',').map((l) => l.trim())
);

// Derive displayed types based on other active filters
export const displayedTypes = derived(activeFilters, ($activeFilters) => {
	const relevantPublications = allPublications.filter((pub) => {
		// Year range filter
		if (
			$activeFilters.yearRange &&
...
export const displayedTags = derived(activeFilters, ($activeFilters) => {
	const relevantPublications = allPublications.filter((pub) => {
		// Type filter
...
export const displayedLanguages = derived(activeFilters, ($activeFilters) => {
	const relevantPublications = allPublications.filter((pub) => {
		// Type filter
		if ($activeFilters.types.length > 0 && !$activeFilters.types.includes(pub.type)) return false;
		// Year range filter
