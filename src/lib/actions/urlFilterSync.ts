...
type ArrayFilterKey = 'setTypes' | 'setTags' | 'setLanguages' | 'setAuthors' | 'setCountries' | 'setProjects';
...
export function urlFilterSync(node: HTMLElement, params: UrlFilterSyncParams) {
    ...
    const unsubscribePage = page.subscribe($page => {
        ...
        const syncArrayFilter = (key: ArrayFilterKey, paramName: string, currentValues: string[] | undefined) => {
            ...
            if (JSON.stringify(valuesFromUrl.sort()) !== JSON.stringify((currentValues || []).sort())) { // Sort for comparison
