...
export function generateBibtex(publication: Publication): string {
    ...
    if (bibtexType === 'collection') {
        if (publication.authors && publication.authors.length > 0) { // These are editors for a collection
    ...
    } else if (publication.authors && publication.authors.length > 0) { // Ensure authors array is not empty
    ...
    if (publication.abstract) bibtexFields.push(`  abstract = {${escapeBibtex(publication.abstract)}}`);
    ...
    if (publication.language) bibtexFields.push(`  language = {${escapeBibtex(publication.language)}}`);
    ...
    switch (bibtexType) {
        case 'article':
            if (publication.journal) bibtexFields.push(`  journal = {${escapeBibtex(publication.journal)}}`);
            ...
            if ((publication as any).issn) bibtexFields.push(`  issn = {${escapeBibtex((publication as any).issn)}}`);
            break;
        case 'book': // For non-edited books
            if (publication.publisher) bibtexFields.push(`  publisher = {${escapeBibtex(publication.publisher)}}`);
            if (publication.placeOfPublication) bibtexFields.push(`  address = {${escapeBibtex(publication.placeOfPublication)}}`);
            ...
            if (publication.pageCount && publication.pageCount > 0) bibtexFields.push(`  pages = {${publication.pageCount}}`);
            break;
        ...
        case 'phdthesis':
        case 'mastersthesis':
            if (publication.university) bibtexFields.push(`  school = {${escapeBibtex(publication.university)}}`);
            if (publication.placeOfPublication) bibtexFields.push(`  address = {${escapeBibtex(publication.placeOfPublication)}}`); // University's address
            if (publication.advisors && publication.advisors.length > 0) {
                notes.push(`Advisors: ${formatAuthors(publication.advisors)}.`);
            }
            break;
        case 'techreport':
            if ((publication as any).institution || publication.publisher) bibtexFields.push(`  institution = {${escapeBibtex((publication as any).institution || publication.publisher)}}`);
            if ((publication as any).reportNumber) bibtexFields.push(`  number = {${escapeBibtex((publication as any).reportNumber)}}`);
            if (publication.placeOfPublication) bibtexFields.push(`  address = {${escapeBibtex(publication.placeOfPublication)}}`);
            break;
    }
    
    if (publication.tags && publication.tags.length > 0) bibtexFields.push(`  keywords = {${publication.tags.map(escapeBibtex).join(', ')}}`);
