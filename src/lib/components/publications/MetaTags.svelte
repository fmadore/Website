<script lang="ts">
	...
	const metaTags = $derived(
		[
			...
			typeof publication.editors === 'string' &&
			publication.editors
				? publication.editors
						.split(/,\s*|\s+and\s+/)
						.map((name) => name.trim())
						.filter((name) => name)
						.map((editor) => ({ name: 'citation_editor', content: editor, condition: undefined }))
				: []),
			...((publication.type === 'special-issue' ||
				(publication.type === 'book' && publication.isEditedVolume)) &&
			Array.isArray(publication.authors) // For edited volumes, authors are editors
				? publication.authors.map((editor) => ({
						name: 'citation_editor',
						content: editor,
						condition: undefined
					}))
			...
			{
				name: 'citation_firstpage',
				content: publication.pages,
				condition:
					(publication.type === 'article' || publication.type === 'chapter') &&
					!!publication.pages &&
					!publication.pages.includes('-') &&
					/^\d+$/.test(publication.pages.trim())
			},
			{
				name: 'citation_lastpage',
				content: publication.pages,
			...
			// Report
			{
				...
				content: (publication as any).institution || publication.publisher,
				condition: publication.type === 'report'
			},
			{
				name: 'citation_technical_report_number',
				content: (publication as any).reportNumber,
				condition: publication.type === 'report'
			},
			...
			...(publication.type === 'chapter' && Array.isArray(publication.editors)
				? publication.editors.map((editor) => ({
						...
						content: editor,
						condition: undefined
					})) // Or DC.contributor if more appropriate role
				: []),
			...(publication.type === 'chapter' &&
			typeof publication.editors === 'string' &&
			publication.editors
				? publication.editors
						...
						.map((name) => name.trim())
						.filter((name) => name)
						.map((editor) => ({ name: 'DC.creator', content: editor, condition: undefined }))
				: []),
			...((publication.type === 'special-issue' ||
				(publication.type === 'book' && publication.isEditedVolume)) &&
			Array.isArray(publication.authors) // Editors for edited volumes
				? publication.authors.map((editor) => ({
						name: 'DC.creator',
						content: editor,
						condition: undefined
			...
			{
				name: 'DC.identifier',
				content: publication.doi ? `doi:${publication.doi}` : getFullUrl(publication.url)
			},
