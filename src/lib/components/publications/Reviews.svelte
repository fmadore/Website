    ...
    <section class="mt-12 reviews-section">
        ...
        <div class="grid grid-cols-1 gap-8">
            {#each sortedReviews as review}
                <div class="review-card bg-sidebar p-6 rounded-md shadow-sm border border-default hover:shadow-md transition-shadow">
                    ...
                    <div class="journal-info text-sm text-light mb-4">
                        ...
                        {#if review.doi}
                            <div class="doi mt-2">
                                DOI: <a href="https://doi.org/{review.doi}" target="_blank" rel="noopener" class="text-primary hover:underline">{review.doi}</a>
                            </div>
                        {/if}
                    </div>
                    
                    {#if review.excerpt}
                        <blockquote class="excerpt mt-5 pl-6 border-l-4 border-primary-light bg-primary-lightest p-4 rounded-r-md italic text-text">
                            "{review.excerpt}"
                        </blockquote>
                    {/if}
                </div>
