...
- **molecules/**: Simple compositions (BibliographyRow — the finding-aid ledger row shared by the publications, talks and activities indexes —, Breadcrumb, IframeRenderer, Pagination, RelatedItemCard, TagList, RecordLedger — the `.meta-ledger` catalogue entry of a record rail, …)
- **organisms/**: Complex UI (RelatedItemsList)
- **common/**: Shared layout (Footer, Card, ContentBody, PageHeader, PageIntro, ProfileBanner, RecordLayout). `RecordLayout` is the record shell — editorial breadcrumb, masthead, reading column, sticky 380px metadata rail with `main`/`railPrimary`/`railSecondary`/`related`/`deck` snippets — used by every detail route (`/publications/[id]`, `/communications/[id]`, `/activities/[id]`, `/digital-humanities/[id]`) and composed by `research/ResearchProjectLayout` for the project pages. Each route supplies its own rail component (`PublicationRecordRail`, `CommunicationRecordRail`, `ActivityRecordRail`, `DhProjectRail`) built from the `.rail-*` / `.meta-ledger` idioms. There is no shared _index_ layout component: the index pages compose `entity-index.css` with `EntityFilterBar`/`EntityFacetGrid` directly.
- **entity-index/**: The live filter UI (EntityFilterBar, EntityFacetGrid, FacetCombobox — the WAI-ARIA combobox that replaces "All N ↓" expansion on the long facets; its pure matching/ranking lives in `facetSearch.ts`)
...
**Typography — two voices, strictly cast.** **Archivo** (display grotesque: nameplate, h1–h3, section heads, big data numbers), **Newsreader** (news serif: all prose, h4–h5, subtitles, captions, italics), **Spline Sans Mono** (the _data voice_: eyebrows, datelines, counts, nav, filters, chips, DOIs, pagination, `h6` — anything that could be a database column). Never blur the voices: no mono headlines, no serif metadata. The one deliberate exception is `h6`, the smallest heading tier, which is a metadata label and is cast mono/uppercase/letterspaced in `typography.css`. Retired: Fraunces, Spectral, Commissioner, JetBrains Mono — and with them the "knowing exception" clause.
